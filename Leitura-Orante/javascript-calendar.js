'use strict';
let dt = new Date("August 1, 2020 23:15:00");

function renderDate() {
    let currentDate=''
    // For PRoduction
if(document.location.pathname.slice(31).length == 5){
     currentDate=document.location.pathname.slice(31).slice(0,1)
}
if(document.location.pathname.slice(31).length == 6){
   currentDate=document.location.pathname.slice(31).slice(0,2)
}
// For Development
// if(document.location.pathname.slice(38).length == 5){
//      currentDate=document.location.pathname.slice(38).slice(0,1)
// }
// if(document.location.pathname.slice(38).length == 6){
//    currentDate=document.location.pathname.slice(38).slice(0,2)
// }
    let dateString = new Date();

    dt.setDate(1);
    let day = dt.getDay();

    let endDate = new Date(
        dt.getFullYear(),
        dt.getMonth() + 1,
        0
    ).getDate();

    let prevDate = new Date(
        dt.getFullYear(),
        dt.getMonth(),
        0
    ).getDate();

    let today = new Date();

    let months = [
        "Janeiro",
        "Fevereiro",
        "Março",
        "Abril",
        "Maio",
        "Junho",
        "Julho", 
        "Agosto",
        "Setembro",
        "Outubro",
        "Novembro",
        "Dezembro"
    ];

    document.getElementById("icalendarMonth").innerHTML = months[dt.getMonth()] ;
    // document.getElementById("icalendarDateStr").innerHTML = dateString.toDateString();

    let cells = "";
    let countDate = 0;

    for (let x = day; x > 0; x--) {
        cells += "<div class='icalendar__prev-date'>" + (prevDate - x + 1) + "</div>";
    }
    for (let i = 1; i <= endDate; i++) {
        if (i == currentDate && months[dt.getMonth()] == 'Agosto' ) {
            console.log(currentDate)
            cells += "<a class='icalendar__today'>" + i + "</a>";
        } else {
            if(months[dt.getMonth()] == 'Agosto'){
                console.log(months[dt.getMonth()] )
                cells += `<a class='a-date' href="leitura-orante-`+i+`.html">`+i+`</a>`;
                // document.getElementsByClassName('a-date').setAttribute('href',`../Leitura-Orante/leitura-orante-`+i+`.html`)
            }
            else if(months[dt.getMonth()] == "Setembro" ){
                console.log(months[dt.getMonth()] )
                cells += `<a class='a-date' href="../Leitura-Orante-September/leitura-orante-`+i+`.html">`+i+`</a>`;
            }else if(months[dt.getMonth()] == "Outubro" ){
                console.log(months[dt.getMonth()] )
                cells += `<a class='a-date' href="../Leitura-Orante-October/leitura-orante-`+i+`.html">`+i+`</a>`;
            }
       
            else{
                cells += `<a class='a-date'>`+i+`</a>`;
            }
            
            

        }

        countDate = i;
    }

    let reservedDateCells = countDate + day + 1;
    for (let j1 = reservedDateCells, j2 = 1; j1 <= 42; j1++, j2++) {
        cells += "<div class='icalendar__next-date'>" + j2 + "</div>";
    }

    document.getElementsByClassName("icalendar__days")[0].innerHTML = cells;
    
}

renderDate();


function moveDate(param) {
    if (param === 'prev') {
        dt.setMonth(dt.getMonth() - 1);
    } else if (param === 'next') {
        dt.setMonth(dt.getMonth() + 1);
    }

    renderDate();
}
const click=()=>{
    console.log('as')
}